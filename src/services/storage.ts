...
export class OsonStorageService {
  ...
  static getConversationMessages(conversationId?: string): ConversationMessage[] {
    ...
    return list.filter(m => m.conversation_id === conversationId || m.scenario_id === conversationId);
  }
  ...
  static saveConversationMessage(conversationId: string, msg: Partial<ConversationMessage>): ConversationMessage {
    const list = getStorage<ConversationMessage[]>(KEYS.CONVERSATIONS, []);
    const newMsg: ConversationMessage = {
      id: 'msg-' + Date.now() + '-' + Math.random().toString(36).substr(2, 4),
